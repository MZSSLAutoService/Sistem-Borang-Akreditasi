...
use App\ModelLKPS\ModelLKPSCover;
use App\ModelLKPS\ModelLKPSCover_prodi_upps;
use App\ModelLKPS\ModelKomentar;
...
class LKPSCover extends Controller
{
    public function index() 
    { 
        $data['tb_prodi'] = DB::table('master_prodi')
            ->get();
        $data['led'] = DB::table('lkps_cover_prodi_upps')
            ->join('lkps_cover', 'lkps_cover_prodi_upps.id_cover', '=', 'lkps_cover.id')
            ->select( 'lkps_cover_prodi_upps.*')
            ->where('id_cover',$_GET['upps'])
            ->get();
        $data['id_cover'] = $_GET['upps'];
        return view('LKPS_Cover.index',$data);
        // return view('standar');
    }
    ...
    { 
        $data['tb_prodi'] = DB::table('master_prodi')
            ->get();
        return view('LKPS_Cover.add', $data);
    }

    public function LKPSCoverPost(Request $request)
    {
        ...
        $cover=DB::table('lkps_cover')
                    ->rightJoin('users','lkps_cover.id_prodi','=','users.id_prodi')
                    ->where('users.id',Session::get('id'))
                    ->first();
        $data1 = new ModelLKPSCover();  
        ...
        if( $data1->save()){
            ...
            ModelLKPSCover_prodi_upps::insert($arr);
            // dd($arr);
        }
        //END ARRAY
    ...
    public function store(Request $request)
    {
        ...
        $data1->id_coverlkps = $request->id_cover;
    ...
    {   
        $dt_arr = ModelLKPSCover_prodi_upps::where('id_cover',$id)->get();
        return view('coverlkps.index',compact('dt_arr'));
    }

    public function edit($id)
    {
        $led_cover = ModelLKPSCover::find($id);
        $led_cover_upps = ModelLKPSCover_prodi_upps::all();
        $query = ModelLKPSCover_prodi_upps::select('id')->where('id_cover', $id)->get();
        return view('coverlkps', compact('lkps_cover','lkps_cover_upps')); 
    }
    ...
    public function update(Request $request)
    {
        ...
        $cek=DB::table('lkps_cover')->where('id',$request->id)->update($data);
        if($cek){ 
            return redirect()->back()->with('edit-success', 'Data Penutup telah diubah');
        }else{
            return redirect()->back()->with('edit-gagal', 'Data Penutup gagal diubah');
    ...
    $data = ModelLKPSCover::where('id',$id)->first();
    ...
    ModelLKPSCover::where('id',$id)->delete();
    
    if($data){
        // $data1 =DB::table('led_cover')->where('id_prodi',Session::get('id_prodi'))->first();


        DB::table('lkps_cover_prodi_upps')
                    ->where('id_cover', $id)->delete();
    }

        //Redirect to Add
        return redirect(url('tambahcoverlkps'))->with('success', 'Data Cover telah dihapus');
    }
}
